...
function aceptarAltaCliente() {
    ...
    let apellidos = frmAltaCliente.txtApellidos.value.trim();
    let oCliente = new Cliente(dniCliente, nombre, apellidos);
    let respuesta = oAgencia.altaCliente(oCliente);
...
function aceptarAltaAlojamiento() {
    let idAlojamiento = siguienteCodigo(oAgencia.alojamientos); //Si validásemos los datos del formulario, inicializaríamos idAlojamiento tras la validación para no desperdiciar números de id. Asumimos que los datos se introducen con un formato válido.
    let numPersonas = parseInt(frmAltaAlojamiento.txtNumPersonas.value.trim());
    ...
    let dormitorios = parseInt(frmAltaAlojamiento.txtNumDormitorios.value.trim());
    let oAlojamiento;
...
function aceptarAltaReserva() {
    ...
    let alojamientos = [];
    let alojamientosAux = [...frmAltaReserva.comboAlojamientos.options].filter(x => x.selected).map(x => x.value); //array auxiliar con los idAlojamiento de los alojamientos seleccionados.
    ...
    let fechaInicio = new Date(frmAltaReserva.dateFechaInicio.value); 
    let fechaFin = new Date(frmAltaReserva.dateFechaFin.value);
    let respuesta;
    let fechaInicioMs = Date.parse(fechaInicio.toLocaleDateString()); //Fecha inicio en milisegundos
    let fechaFinMs = Date.parse(fechaFin.toLocaleDateString()); //Fecha fin en milisegundos
    let fechaActual = Date.parse((new Date()).toLocaleDateString()); //Fecha actual en milisegundos
...
function aceptarBajaReserva() {
    let idReserva = parseInt(frmBajaReserva.comboReservas.value);
    let respuesta = oAgencia.bajaReserva(idReserva);
...
function mostrarListadoClientes() {
    let sListado = oAgencia.listadoClientes();
    let tituloListado = "Listado de clientes";
    let tituloPag = "Listado clientes";
...
function mostrarListadoAlojamientos() {
    let sListado = oAgencia.listadoAlojamientos();
    let tituloListado = "Listado de alojamientos";
    let tituloPag = "Listado alojamientos";

    mostrarListadoAux(sListado, tituloListado, tituloPag);
}

function AceptarListadoReservasPorFecha() {
    let fechaInicio = new Date(frmListadoReservasPorFecha.dateFechaInicio.value); 
    let fechaFin = new Date(frmListadoReservasPorFecha.dateFechaFin.value);
    let sListado = oAgencia.listadoReservas(fechaInicio, fechaFin);
    let tituloListado = "Listado de reservas entre " + fechaInicio.toLocaleDateString() 
                        + " y " + fechaFin.toLocaleDateString();

    let tituloPag = "Listado reservas por fecha";
...
function AceptarListadoReservasPorCliente() {
    let cliente = parseInt(frmListadoReservasPorCliente.comboClientes.value); //dni
    let sListado = oAgencia.listadoReservasCliente(cliente);
    let tituloListado = "Listado de reservas del cliente con DNI " + cliente;
    let tituloPag = "Listado reservas por cliente";
...
function mostrarListadoHabitacionesConDesayuno() {
    let sListado = oAgencia.listadoHabitacionesConDesayuno();
    let tituloListado = "Listado de habitaciones con desayuno";
    let tituloPag = "Listado habitaciones desayuno";
